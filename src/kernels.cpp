...
__global__ void associateAllBIG2(float* payloads, int sizePayload, float* measures, 
                                ...
                                float* curr_dist)
{
    ...
    for (int yIdx = threadIdx.y; yIdx < numMeasures; yIdx += blockDim.y)   
    {
        ...
        curr_dist[yIdx*MAX_KF+threadIdx.x] = is_invalid * 1e5f +
		                               (p_z_predict[0]-meas[0])*(p_z_predict[0]-meas[0]) + 
                                       (p_z_predict[2]-meas[2])*(p_z_predict[2]-meas[2]) + 
                                       (p_z_predict[4]-meas[4])*(p_z_predict[4]-meas[4]);
    }
    ...
    for (int yIdx = threadIdx.y; yIdx < numMeasures; yIdx += blockDim.y)
    {
        if (curr_dist[yIdx*MAX_KF+threadIdx.x] < local_min)
        {
            local_min = curr_dist[yIdx*MAX_KF+threadIdx.x];
            local_min_idx = yIdx;
        }
    }

    curr_dist[threadIdx.y*MAX_KF+threadIdx.x] = local_min;
    min_indexes[threadIdx.y][threadIdx.x] = local_min_idx;
    __syncthreads();

    if (threadIdx.y < 4)
    {
        if (curr_dist[(threadIdx.y + 4)*MAX_KF+threadIdx.x] < curr_dist[threadIdx.y*MAX_KF+threadIdx.x])
	    {
	        curr_dist[threadIdx.y*MAX_KF+threadIdx.x] = curr_dist[(threadIdx.y + 4)*MAX_KF+threadIdx.x];
    ...
    if (threadIdx.y < 2)
    {
        if (curr_dist[(threadIdx.y + 2)*MAX_KF+threadIdx.x] < curr_dist[threadIdx.y*MAX_KF+threadIdx.x])
        {
            curr_dist[threadIdx.y*MAX_KF+threadIdx.x] = curr_dist[(threadIdx.y + 2)*MAX_KF+threadIdx.x];
    ...
    if (threadIdx.y < 1)
    {
        if (curr_dist[threadIdx.y*MAX_KF+threadIdx.x] < curr_dist[(threadIdx.y + 1)*MAX_KF+threadIdx.x])
        {
            distThreshold[threadIdx.x] = curr_dist[threadIdx.y*MAX_KF+threadIdx.x];
            associations[threadIdx.x] = min_indexes[threadIdx.y][threadIdx.x];
        }
        else
        {
            distThreshold[threadIdx.x] = curr_dist[(threadIdx.y + 1)*MAX_KF+threadIdx.x];
